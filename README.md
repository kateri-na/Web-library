...
### В базе данных PostgreSQL должно быть две таблицы - Person и Book. Приложение запускается с помощью сервера Tomcat 10.1.28 и использует JDBC Template

Функционал
(все поля должны валидироваться - с помощью @Valid и Spring Validator, если это
требуется):
